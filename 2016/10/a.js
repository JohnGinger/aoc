...
function process(instructions) {
    let bots = getBots(instructions
            .split('\n')
            .filter(x => x[0] === "v"))
        //console.log(bots)

    let moves = getMoves(instructions
            .split('\n')
            .filter(x => x[0] !== "v"))
        // console.log(moves)
    ...
    while (!weHaveTheOnesWeWant(currentBots)) {
        currentBots.forEach(x => {
            ...
            ({
                bots,
                bot
            } = getBot(bots, move.low))

            if (!move.lowOutput) {
                bot.boxes.push(lowBox)
            } else {
                outputs[bot.bot] = outputs[bot.bot] ?
                    outputs[bot.bot].push(lowBox) : [lowBox]
            }

            ({
                bots,
                bot
            } = getBot(bots, move.high))

            if (!move.highOutput) {
                bot.boxes.push(highBox)
            } else {
                outputs[bot.bot] = outputs[bot.bot] ?
                    outputs[bot.bot].push(highBox) : [highBox]
            }
            x.boxes = [];
        })
