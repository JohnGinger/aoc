...
function showStatus() {
    console.log(lights[0].map(x => x > 0 ? "0" :" ").join(""));
    console.log(lights[1].map(x => x > 0 ? "0" :" ").join(""));
    console.log(lights[2].map(x => x > 0 ? "0" :" ").join(""));
    console.log(lights[3].map(x => x > 0 ? "0" :" ").join(""));
    console.log(lights[4].map(x => x > 0 ? "0" :" ").join(""));
    console.log(lights[5].map(x => x > 0 ? "0" :" ").join(""));
    console.log("");
}
...
function getVoltage(input) {
    ...
    lines.forEach(l => {
        ...
        for (let i = 0; i < 100000; i++) {
            require('crypto').createHash('md5').update("test" + i);
        }
        console.log(l);
        doOperation(l);
        showStatus();
