use crate::aoc_util::{iterate_input_lines, PuzzleLines};
use std::fmt;
...
fn map_pattern(line: &[bool]) -> String {
    line.into_iter()
        .map(|&x| if x { "#" } else { "." })
        .collect()
}
...
fn get_pattern(line: &str) -> Vec<Pattern> {
...
fn get_flipped_y(pattern: &Pattern) -> Pattern {
    ...
    let mut new_pattern = pattern.clone();
    for y in 0..size {
        new_pattern.check_points[y][..size]
            .clone_from_slice(&pattern.check_points[(size - 1 - y) % size][..size])
    }
    new_pattern
}
...
fn divisible_by_two(grid: &[Vec<bool>]) -> bool {
    grid.len() % 2 == 0
}

fn pattern_matches(grid: &[Vec<bool>], start_x: usize, start_y: usize, pattern: &Pattern) -> bool {
...
pub fn run() {
    let lines = iterate_input_lines(21);
